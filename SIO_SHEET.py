"""
SIO-Fold v4.1: Beta-Sheet Tuned
Status: SPECIALIZED / HIGH STABILITY
Date: December 2025
Author: DULA2025

Updates:
- Initialization tuned for Beta-Sheets (Delta 0.45-0.60).
- Larger Lattice Scaling (4.0-4.5) to prevent "Knots" in long chains.
- Hybrid PDBFixer/RDKit Robustness included.
"""
...
log = logging.getLogger("SIO-Sheet")
...
def run_vacuum_physics(pdb_path, output_path):
    try:
        ...
        # High Friction for Sheet Stability
        friction_std = 3.0/unit.picosecond
...
class VortexState:
    def __init__(self, n_nodes):
        # TUNED FOR SHEETS: High Delta (0.45-0.60), High Scaling (4.0-4.5)
        self.delta = np.random.uniform(0.45, 0.60, n_nodes) 
        self.alpha = np.random.uniform(0.2, 0.8, n_nodes) 
        self.theta = np.random.uniform(-np.pi, np.pi)     
        self.scaling = np.random.uniform(4.0, 4.5)        
    ...
    def mutate(self):
        if np.random.rand() < 0.5:
            ...
            self.delta[idx] = np.clip(self.delta[idx] + np.random.normal(0, 0.05), 0.35, 0.65)
...
def main():
    ...
    log.info(f"--- SIO-FOLD v4.1 (Sheet Tuned) ---")
        ...
        for i, vortex in enumerate(population):
            ...
            try:
                ...
                if best_state is None or energy < best_state.energy:
                    best_state = vortex
                    milestone = f"SIO_SHEET_E{energy:.0f}.pdb"
                    import shutil
                    shutil.copy(final_pdb, milestone)
                    log.info(f"STEP {step}: Sheet Attractor (E={energy:.2f}) -> {milestone}")
